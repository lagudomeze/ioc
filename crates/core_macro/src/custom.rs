...
use syn::{parse_quote, Ident, ImplItem, ItemImpl, Path, Type};
...
const CUSTOM_BEAN_IMPL_ERROR_INFO: &str = "BeanSpec custom impls only allow:
        ...
        if let Some((_, trait_, _)) = &impl_block.trait_ {
            ...
            for item in impl_block.items.iter() {
                match item {
                    ...
                    other => {
                        return Err(Error::custom(CUSTOM_BEAN_IMPL_ERROR_INFO).with_span(&other));
                    }
                }
            }
            ...
            Err(
                Error::custom("Bean attribute can only be used on trait (ioc::BeanSpec) impls")
