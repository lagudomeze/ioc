    ...
    T: 'static + OpenApi,
{
    ...
    if config.static_enable {
        ...
        for (name, mapping) in config.static_mappings.iter() {
            ...
            if mapping.listing {
                endpoint = endpoint.show_files_listing().redirect_to_slash_directory();
            }
    ...
    Server::new(listener)
        .run_with_graceful_shutdown(app, gracefully_shutdown(), Some(config.shutdown_timeout))
        .await?;
    ...
    T: 'static + OpenApi,
{
    ...
    runtime.block_on(async { run_server(api, title, version).await })?;
    Ok(())
}
