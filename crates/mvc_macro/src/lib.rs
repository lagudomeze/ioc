...
use syn::{
    parse_macro_input, parse_quote, spanned::Spanned, visit_mut, visit_mut::VisitMut, Attribute,
    Error, FnArg, ImplItem, ImplItemFn, ItemImpl, TraitItemFn, Type,
};
...
impl VisitMut for ItemImplPatch {
    fn visit_item_impl_mut(&mut self, i: &mut ItemImpl) {
        if self.api_trait_type.is_none() {
...
impl ItemImplPatch {
    ...
    fn new_api_trait(raw_type: &Type) -> ApiTraitInfo {
        let raw_type: Ident = { parse_quote!(#raw_type) };
    ...
    fn is_api_fn(item_fn: &ImplItemFn) -> bool {
        ...
        item_fn.sig.asyncness.is_some()
            && item_fn.attrs.iter().any(|attr| attr.path().is_ident("oai"))
    }
