...
compile_error!(
    "feature \"env_logger\" and feature \"tracing_log\" cannot be enabled at the same time"
);
...
mod tracing_log {
    use tracing_subscriber::{
        filter::Directive, filter::LevelFilter, fmt::Formatter, reload::Handle, EnvFilter,
    };
    ...
    impl LogPatcher {
        ...
        pub fn to_string(&self) -> crate::Result<String> {
            let result = self
                .0
                .with_current(|filter: &EnvFilter| filter.to_string())
                .map_err(anyhow::Error::new)?;
            Ok(result)
        }
    }
